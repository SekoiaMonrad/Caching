...
{
    public class ElementCacheConcurrentRefreshesTest
    {
        private readonly ITestOutputHelper _testOutputHelper;

        public ElementCacheConcurrentRefreshesTest([NotNull] ITestOutputHelper testOutputHelper)
        {
        ...
        [Theory]
        [AutoMoqData]
        public async Task TestConcurrentRefreshesArePerformedSequentially(string key)
        {
                                          ...
                                          e => exception = e))
            {
                ...
                await AssertEx.EqualAsync($"{key}-{elementRetrieverCalls}",
                                          () => values.Last(),
        ...
        [Theory]
        [AutoMoqData]
        public async Task TestConcurrentRefreshesArePerformedSequentially_Continously(string key)
        {
            for (int i = 0; i < 100; i++)
            {
                await TestConcurrentRefreshesArePerformedSequentially(key)
                    .ConfigureAwait(false);
            }
        }
    }
