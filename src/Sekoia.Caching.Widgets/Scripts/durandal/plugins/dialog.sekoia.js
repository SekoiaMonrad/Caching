 ...
 * @requires knockout
 */
define(['durandal/system', 'durandal/app', 'durandal/composition', 'durandal/activator', 'durandal/viewEngine', 'jquery', 'knockout', 'linqjs'], function (system, app, composition, activator, viewEngine, $, ko, Enumerable) {
    var contexts = {},
    ...
    var MessageBox = function (message, title, options, autoclose, settings) {
        ...
        this.defaultOption = Enumerable.from(this.options).where(function(o) { return o.isDefault === true; }).firstOrDefault();
    ...
    MessageBox.defaultSettings = { buttonClass: "btn btn-default", primaryButtonClass: "btn-primary", secondaryButtonClass: "", "class": "modal-content messageBox", style: null };
    ...
    */
    ...
    MessageBox.prototype.getButtonClass = function (option, $index) {
        var c = "";

        var isPrimary = option.isPrimary === true
                            ? true
                            : option.isPrimary === false
                                  ? false
                                  : null;
        var isSecondary = option.isSecondary === true
                              ? true
                              : option.isSecondary === false
                                    ? false
                                    : null;
        ...
        if (this.settings) {
            ...
            if (isPrimary === true || (isPrimary !== false && $index() === this.options.length - 1 && this.settings.primaryButtonClass)) {
                if (c.length > 0) {
                    c += " ";
                }
                c += this.settings.primaryButtonClass;
            }
            if (isSecondary === true || (isSecondary !== false && $index() !== this.options.length - 1 && this.settings.secondaryButtonClass)) {
                if (c.length > 0) {
                    c += " ";
                }
        ...
        if (option === this.defaultOption || (!this.defaultOption && $index() === this.options.length - 1)) {
            if (c.length > 0) {
                c += " ";
            }
            c += "autofocus";
    ...
    MessageBox.defaultViewMarkup = [
        '<div data-view="plugins/messageBox" data-bind="css: getClass(), style: getStyle()" class="modal-dialog">',
            '<div class="modal-content">',
                ...
                '<div class="modal-footer">',
                    '<!-- ko foreach: options.slice(0).reverse() -->',
                        '<button data-bind="click: function () { $parent.selectOption($parent.getButtonValue($data)); }, text: $parent.getButtonText($data), css: $parent.getButtonClass($data, $index)"></button>',
                    '<!-- /ko -->',
    ...
    dialog = {
        ...
        getContext: function (name) {
            return contexts[name || 'bootstrapModal'];
        },
        /**
        ...
        show: function (obj, activationData, context) {
            var that = this;
            var dialogContext = contexts[context || 'bootstrapModal'];

            return system.defer(function (dfd) {
                ensureDialogInstance(obj).then(function (instance) {
                    if (instance.autoclose === undefined)
